...
async function getPhotosQuery() {
  if (inputSearch.value !== "") {
    ...
    const url = `https://api.unsplash.com/search/photos?client_id=eQ31g4OVoE2n40OnuWIHfBZ6yw2tHPnJ85C-QC6waoQ&per_page=9&orientation=landscape&query=${inputSearch.value}`;
    try {
      const response = await fetch(url);
      const data = await response.json();
      const photos = data.results;
